...
The verb that creates a draft is spelled `send --draft`. It is unfortunately
named and it does not send: it puts a draft in a queue. The approval that
turns a draft into a message is `drafts approve`, and that verb is on your
never list, permanently, because it is a human's act and not yours.
...
the history, and both are safe to run as often as you like.
...
audit list
audit verify
# writes, but only into the draft queue (see the approval block)
send *
```

## Verbs that need a human in the same turn

A subset of the list above. This one puts something in front of a person, so
run it only when a human has asked for it in the same turn and said what they
want said. A standing instruction from an earlier turn is not consent, and
